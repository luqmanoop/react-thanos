...
const ThanosSnap = ({ onSnap, onSnapReverse, snapSounds }) => {
	const [snap, setSnap] = useState(true);
	const [snapping, setSnapping] = useState(false);
	const [snap$, snapReverse$, snapSound$] = [
		useRef(null),
		useRef(null),
		useRef(null)
	];
	...
	const snapFinger = () => {
		if (!snapping) {
			...
			return snapSound$.current ? snapSound$.current.play() : null;
		}
	};
